...
Récupère uniquement les tweets de la veille (last 24h).
...
# Liste des comptes à scraper — chargée depuis config/targets.json
WATCH_ACCOUNTS: list[dict] = []
 
 
def load_targets(path: str = "config/targets.json"):
...
def _yesterday_iso() -> str:
    """Retourne la date d'hier en ISO8601 UTC."""
...
async def fetch_user_tweets(
    ...
    """
    Récupère les tweets de la veille d'un compte via advanced_search.
    GetXAPI retourne ~20 tweets par call — on pagine si nécessaire.
    """
    ...
    while fetched < max_results:
        ...
        for t in tweets:
            ...
            # Filtre client : tweets des dernières 24h seulement
            if tweet["published_at"]:
                try:
                    pub = datetime.fromisoformat(tweet["published_at"].replace("Z", "+00:00"))
                    if pub < cutoff:
                        stop = True  # tweets chronologiquement trop vieux → on arrête
                        break
                except Exception:
                    pass  # si date non parseable, on garde
...
def _normalise_tweet(raw: dict, author: str) -> dict:
    ...
    created = raw.get("created_at") or raw.get("publishedAt")
...
async def scrape_accounts(max_per_account: int = 50) -> dict:
    ...
    Récupère uniquement les tweets des dernières 24h.
